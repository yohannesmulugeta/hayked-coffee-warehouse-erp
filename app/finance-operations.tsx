...
import { AlertTriangle, Banknote, CalendarDays, Check, Eye, FileCheck2, LockKeyhole, Printer, ReceiptText, Search, ShieldCheck, WalletCards } from "lucide-react";
import { FormEvent, useEffect, useState } from "react";
import { allocatePayment, type StorageCategory } from "./finance-rules";
import { loadFinanceData, quoteStorageBilling, recordPayment as postPayment, runStorageBilling, type FinanceData, type StorageQuote } from "@/lib/erp-data";
...
export function FinanceOperations() {
  ...
  const inBillingPeriod = (date: string | null) => Boolean(date && (!billingFrom || date.slice(0, 10) >= billingFrom) && (!billingTo || date.slice(0, 10) <= billingTo));
  const visibleServices = (data?.serviceEvents ?? []).filter((item) => !item.invoice_id && item.status === "UNBILLED" && (!billingClientId || item.client_id === billingClientId) && (serviceType === "All" || item.service_type === serviceType) && inBillingPeriod(item.created_at) && `${clientById.get(item.client_id)} ${item.description} ${item.service_type}`.toLowerCase().includes(billingQuery.trim().toLowerCase()));
  ...
  const clientAccounts = clients.map((client) => {
    ...
    const periodInvoices = invoices.filter((row) => inBillingPeriod(row.invoice.issued_on));
    const periodServices = services.filter((row) => inBillingPeriod(row.created_at));
    const periodPayments = payments.filter((row) => inBillingPeriod(row.paid_at));
    const openInvoices = invoices.filter((row) => row.outstanding > 0);
    return {
      client, invoices, services, payments,
      billed: periodInvoices.reduce((sum, row) => sum + Number(row.invoice.total_etb), 0),
      paid: periodPayments.reduce((sum, row) => sum + Number(row.amount_etb), 0),
      outstanding: invoices.reduce((sum, row) => sum + row.outstanding, 0),
      unbilled: periodServices.reduce((sum, row) => sum + Number(row.total_amount), 0),
      oldestDue: openInvoices.map((row) => row.invoice.due_on).filter(Boolean).sort()[0] ?? null,
      overdue: openInvoices.some((row) => row.overdue > 0),
    };
  });
  const visibleClientAccounts = clientAccounts.filter((account) => {
    const matchesClient = !billingClientId || account.client.id === billingClientId;
    const matchesSearch = `${account.client.code} ${account.client.legal_name}`.toLowerCase().includes(billingQuery.trim().toLowerCase());
    const matchesStatus = accountStatus === "All" || (accountStatus === "Owing" && account.outstanding > 0) || (accountStatus === "Overdue" && account.overdue) || (accountStatus === "Clear" && account.outstanding === 0);
    return matchesClient && matchesSearch && matchesStatus;
  }).sort((a, b) => accountSort === "Client A to Z" ? a.client.legal_name.localeCompare(b.client.legal_name) : accountSort === "Oldest unpaid" ? (a.oldestDue ?? "9999-12-31").localeCompare(b.oldestDue ?? "9999-12-31") : b.outstanding - a.outstanding);
  const filteredInvoiceRows = invoiceRows.filter((row) => (!billingClientId || row.invoice.client_id === billingClientId) && inBillingPeriod(row.invoice.issued_on) && `${row.invoice.invoice_number} ${clientById.get(row.invoice.client_id)}`.toLowerCase().includes(billingQuery.trim().toLowerCase()) && (accountStatus === "All" || (accountStatus === "Owing" && row.outstanding > 0) || (accountStatus === "Overdue" && row.outstanding > 0 && row.overdue > 0) || (accountStatus === "Clear" && row.outstanding === 0)));
  ...
  const visibleTariffLines = data?.tariffLineItems.filter((item) => item.tariff_version_id === visibleTariff?.id) ?? [];
  ...
  const changedStorageRows = storageRows.filter((row, index) => index === 0 || index === storageRows.length - 1 || row.movementBags !== 0 || row.rate !== storageRows[index - 1]?.rate);
  const displayedStorageRows = showAllStorageDays ? storageRows : changedStorageRows;
  const selectedInvoicePayments = data?.payments.filter((payment) => payment.invoice_id === selectedInvoiceRow?.invoice.id) ?? [];

  function clearStorageQuote() { setStorage(null); setStorageError(""); setShowAllStorageDays(false); }
  function selectClient(clientId: string) { setSelectedClientId(clientId); setSelectedLotId(""); clearStorageQuote(); }

  async function calculateStorageQuote() {
    if (!selectedClient || !selectedLot || !visibleTariff || periodStart > periodEnd) return;
    setStorageBusy(true); setStorageError(""); setStorage(null);
    try {
      setStorage(await quoteStorageBilling({ clientId: selectedClient.id, lotId: selectedLot.id, category, periodStart, periodEnd, tariffVersion: visibleTariff.version_code, certified }));
    } catch (error) { setStorageError(error instanceof Error ? error.message : "The storage quote could not be calculated."); }
  ...
  async function postStorage() {
    ...
    try {
      const runId = await runStorageBilling({ clientId: selectedClient.id, lotId: selectedLot.id, category, periodStart, periodEnd, tariffVersion: visibleTariff.version_code, certified });
  ...
  return <div className="module-page finance-page">
    ...
    {["Client Accounts", "Unbilled Services", "Invoices"].includes(tab) && <section className="filter-toolbar billing-filter-toolbar" aria-label="Billing filters"><label>From date<input type="date" value={billingFrom} onChange={(event) => setBillingFrom(event.target.value)} /></label><label>To date<input type="date" min={billingFrom} value={billingTo} onChange={(event) => setBillingTo(event.target.value)} /></label><label>Client<select value={billingClientId} onChange={(event) => setBillingClientId(event.target.value)}><option value="">All clients</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.legal_name}</option>)}</select></label><label className="filter-search"><Search size={15} /><input value={billingQuery} onChange={(event) => setBillingQuery(event.target.value)} placeholder="Search client or reference" aria-label="Search billing" /></label>{tab !== "Unbilled Services" && <label>Account status<select value={accountStatus} onChange={(event) => setAccountStatus(event.target.value)}><option>All</option><option>Owing</option><option>Overdue</option><option>Clear</option></select></label>}{tab === "Unbilled Services" && <label>Service type<select value={serviceType} onChange={(event) => setServiceType(event.target.value)}><option>All</option>{serviceTypes.map((value) => <option key={value}>{value}</option>)}</select></label>}{tab === "Client Accounts" && <label>Sort by<select value={accountSort} onChange={(event) => setAccountSort(event.target.value)}><option>Outstanding high to low</option><option>Oldest unpaid</option><option>Client A to Z</option></select></label>}<button className="secondary-button" type="button" onClick={() => { setBillingFrom(monthStart); setBillingTo(today); setBillingClientId(""); setBillingQuery(""); setAccountStatus("All"); setAccountSort("Outstanding high to low"); setServiceType("All"); }}>Reset</button><p className="filter-help">Date filters change period activity. Current outstanding always shows the complete account balance.</p></section>}

    {tab === "Client Accounts" && <section className="record-panel"><div className="record-table client-account-cols"><div className="table-head"><span>Client</span><span>Period unbilled</span><span>Period invoiced</span><span>Period paid</span><span>Current outstanding</span><span>Oldest unpaid</span><span>Action</span></div>{visibleClientAccounts.map((account) => <div className="clickable-row" role="button" tabIndex={0} key={account.client.id} onClick={() => setAccountClientId(account.client.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setAccountClientId(account.client.id); }}><span><strong>{account.client.legal_name}</strong><small>{account.client.code}</small></span><span>ETB {account.unbilled.toLocaleString()}</span><span>ETB {account.billed.toLocaleString()}</span><span>ETB {account.paid.toLocaleString()}</span><span><strong className={account.outstanding > 0 ? "negative" : "positive"}>ETB {account.outstanding.toLocaleString()}</strong></span><span>{account.oldestDue ?? "-"}<small>{account.overdue ? "Overdue" : account.outstanding > 0 ? "Open" : "Clear"}</small></span><span><button className="table-action" type="button" onClick={(event) => { event.stopPropagation(); setAccountClientId(account.client.id); }}><Eye size={14} />Full account</button></span></div>)}</div>{visibleClientAccounts.length === 0 && <p className="empty-result">No client accounts match these filters.</p>}</section>}

    {tab === "Unbilled Services" && <><section className="record-panel"><div className="record-table unbilled-service-cols"><div className="table-head"><span>Date</span><span>Client</span><span>Service</span><span>Reference</span><span>Amount</span><span>Status</span></div>{visibleServices.map((item) => <div key={item.id}><span>{item.created_at.slice(0, 10)}</span><span>{clientById.get(item.client_id) ?? "Unknown client"}</span><span><strong>{item.service_type.replaceAll("_", " ")}</strong><small>{item.description}</small></span><span className="reference">{item.reference_id.slice(0, 8).toUpperCase()}</span><span>ETB {Number(item.total_amount).toLocaleString()}<small>{Number(item.quantity).toLocaleString()} x {Number(item.unit_price).toLocaleString()}</small></span><Status value={item.status} /></div>)}</div>{visibleServices.length === 0 && <p className="empty-result">No unbilled services match the selected filters.</p>}</section><div className="locked-action"><LockKeyhole size={16} /><span><strong>Invoice creation remains controlled</strong><small>Selecting and issuing service events stays disabled until verified tax mappings and invoice rules are confirmed.</small></span><button type="button" disabled>Create invoice</button></div></>}

    {tab === "Invoices" && <section className="record-panel"><div className="record-table invoice-workspace-cols"><div className="table-head"><span>Invoice</span><span>Client</span><span>Issue / due</span><span>Total</span><span>Paid</span><span>Outstanding</span><span>Status</span><span>Action</span></div>{filteredInvoiceRows.map((item) => <div key={item.invoice.id}><span className="reference">{item.invoice.invoice_number}</span><span>{clientById.get(item.invoice.client_id) ?? "Unknown client"}</span><span>{item.invoice.issued_on ?? "Draft"}<small>Due {item.invoice.due_on ?? "-"}{item.overdue > 0 && item.outstanding > 0 ? ` - ${item.overdue} days overdue` : ""}</small></span><span>ETB {Number(item.invoice.total_etb).toLocaleString()}</span><span>ETB {item.paid.toLocaleString()}</span><span><strong>ETB {item.outstanding.toLocaleString()}</strong></span><Status value={item.outstanding === 0 ? "PAID" : item.overdue > 0 ? "OVERDUE" : item.invoice.status} /><span><button className="table-action" type="button" disabled={item.outstanding === 0} onClick={() => { setSelectedInvoiceId(item.invoice.id); setPaymentAmount(item.outstanding); setTab("Payments"); }}>Record payment</button></span></div>)}</div>{filteredInvoiceRows.length === 0 && <p className="empty-result">No invoices match these filters.</p>}</section>}
    ...
    {tab === "Storage Review" && <div className="storage-layout"><section className="storage-controls"><header><CalendarDays size={19} /><div><h2>Storage charge review</h2><p>{selectedLot ? `${selectedLot.lot_number} - ${selectedLot.bag_count} current bags` : "Select a client and lot"}</p></div></header><div className="control-fields"><label>Client<select value={selectedClientId} onChange={(event) => selectClient(event.target.value)}><option value="" disabled>Select client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.legal_name}</option>)}</select></label><label>Lot<select value={selectedLotId} onChange={(event) => { setSelectedLotId(event.target.value); clearStorageQuote(); }}><option value="" disabled>Select lot</option>{clientLots.map((lot) => <option key={lot.id} value={lot.id}>{lot.lot_number}</option>)}</select></label><label>Period start<input type="date" value={periodStart} onChange={(event) => { setPeriodStart(event.target.value); clearStorageQuote(); }} /></label><label>Period end<input type="date" min={periodStart} value={periodEnd} onChange={(event) => { setPeriodEnd(event.target.value); clearStorageQuote(); }} /></label><label>Storage category<select value={category} onChange={(event) => { setCategory(event.target.value as StorageCategory); clearStorageQuote(); }}><option value="NO_PROCESSING">Stored without processing</option><option value="WAITING_PROCESSING">Waiting for processing</option><option value="PROCESSED_EXPORT">Processed export coffee</option><option value="GRADE_IMPROVEMENT">Grade-improvement coffee</option><option value="REJECT">Reject coffee</option><option value="EMPTY_BAGS">Empty bags</option></select></label><label className="inline-control"><input type="checkbox" checked={certified} onChange={(event) => { setCertified(event.target.checked); clearStorageQuote(); }} />Certified coffee</label></div><div className="tariff-source"><ShieldCheck size={16} /><span><strong>{tariff ? `${tariff.version_code} verified rate source` : "No verified tariff"}</strong><small>The database—not the browser—selects the rate for every day.</small></span></div><button className="secondary-button calculate-storage-button" type="button" onClick={calculateStorageQuote} disabled={!selectedLot || !tariff || storageBusy || periodStart > periodEnd}><CalendarDays size={16} />{storageBusy ? "Calculating..." : "Calculate daily charges"}</button>{storageError && <div className="request-form-error" role="alert">{storageError}</div>}<div className="storage-summary"><div><span>Billable days</span><strong>{storage ? storage.rows.filter((row) => row.rate > 0).length.toLocaleString() : "-"}</strong></div><div><span>Billable bag-days</span><strong>{storage?.billableBagDays.toLocaleString() ?? "-"}</strong></div><div><span>Calculated amount</span><strong>ETB {storage?.amount.toLocaleString() ?? "-"}</strong></div></div><button className="primary-button" type="button" onClick={postStorage} disabled={!storage || !tariff || storagePosted}><FileCheck2 size={16} />{storagePosted ? "Billing run already posted" : storage ? "Post verified storage run" : "Calculate before posting"}</button></section><section className="storage-explanation"><header><div><h2>Daily calculation</h2><p>{showAllStorageDays ? "Every day in the selected period" : "Only the first, last, movement and rate-change days"}</p></div>{storageRows.length > 0 && <button className="secondary-button" type="button" onClick={() => setShowAllStorageDays((value) => !value)}>{showAllStorageDays ? "Show changes only" : `Show every day (${storageRows.length})`}</button>}</header><div className="storage-table daily-storage-table"><div><span>Date</span><span>Opening</span><span>Change</span><span>Closing</span><span>Age</span><span>Rate</span><span>Daily amount</span><span>Why shown</span></div>{displayedStorageRows.map((row) => { const sourceIndex = storageRows.findIndex((item) => item.date === row.date); const previous = storageRows[sourceIndex - 1]; const reason = row.movementBags !== 0 ? `Stock movement${row.references.length ? ` · ${row.references.join(", ")}` : ""}` : previous && row.rate !== previous.rate ? `Rate changed from ETB ${previous.rate}` : sourceIndex === 0 ? "First selected day" : sourceIndex === storageRows.length - 1 ? "Last selected day" : "Daily detail"; return <div className={row.movementBags !== 0 || (previous && row.rate !== previous.rate) ? "changed-day" : ""} key={row.date}><span>{row.date}</span><span>{Number(row.openingBags).toLocaleString()}</span><span className={row.movementBags < 0 ? "negative" : row.movementBags > 0 ? "positive" : ""}>{row.movementBags > 0 ? "+" : ""}{Number(row.movementBags).toLocaleString()}</span><span>{Number(row.closingBags).toLocaleString()}</span><span>Day {row.ageDay}</span><span><strong>ETB {Number(row.rate).toLocaleString()}</strong></span><span>ETB {Number(row.amount).toLocaleString()}</span><span><small>{reason}</small></span></div>; })}</div>{storage && displayedStorageRows.length === 0 && <p className="empty-result">No storage days are available for this period.</p>}{!storage && !storageBusy && <p className="empty-result">Choose the client, lot and dates, then calculate the daily charges.</p>}</section></div>}
