...
export function ProcessingOperations() {
  ...
  const staff = data?.profiles.filter((item) => item.active) ?? [];
  const selectedClient = clients.find((item) => item.id === selectedClientId);
  const eligibleLots = lots.filter((lot) => lot.client_id === selectedClientId && Number(lot.quantity_kg) > 0 && !["DISPATCHED", "CLOSED", "REVERSED", "IN_PROCESS"].includes(lot.status));
  ...
  const availableKg = new Map(lots.map((lot) => [lot.id, Number(lot.quantity_kg)]));
  const availableBags = new Map(lots.map((lot) => [lot.id, Number(lot.bag_count)]));
  ...
  const orders: Order[] = (data?.orders ?? []).filter((item) => ["IN_PROCESS", "POSTED"].includes(item.status)).map((item) => {
    ...
    return { databaseId: item.id, id: item.order_number, position: item.queue_position, client: clientById.get(item.client_id) ?? "Unknown client", lot: lot?.lot_number ?? "Multiple source lots", coffeeType: lot?.coffee_type === "WASHED" ? "Washed" : "Unwashed / UG", grade: requestById.get(item.request_id ?? "")?.grade ?? "-", inputBags: inputByOrder.get(item.id) ?? 0, inputKg: Number(item.input_kg), received: "-", readiness: "READY", note: "", status: item.status === "POSTED" ? "COMPLETED" : "IN_PROCESS", completionNumber: item.completion_number, machine: intake?.machine_line ?? "-", startedAt: item.started_at };
  });
  ...
  function selectRequestLot(key: number, lotId: string) {
    ...
    updateRequestLine(key, { lotId, requestedBags: Number(lot?.bag_count ?? 0), requestedKg: Number(lot?.quantity_kg ?? 0) });
  }
  ...
  async function createRequest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`${requestNumber} saved with ${lines.length} source line${lines.length === 1 ? "" : "s"}.`);
  ...
  async function submitIntake(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Processing intake could not be posted."); }
  }
