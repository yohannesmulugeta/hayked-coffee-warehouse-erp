...
import { AlertTriangle, ArrowRight, Check, Clock3, Factory, FileCheck2, Minus, PackageCheck, Pencil, Plus, Scale, Search, Send, ShieldCheck, Trash2, ThumbsDown, ThumbsUp, X } from "lucide-react";
...
import { DetailGrid, EvidenceUploader, RecordDetailDrawer, WorkflowGuide } from "./workflow-ui";
...
type RequestLineDraft = { key: number; lot: EligibleProcessingLot; requestedBags: number; requestedKg: number };
...
const sourceTypeLabels: Record<SourceType, string> = { ARRIVAL: "Arrival", REJECT: "Reject", PROCESSED: "Processed" };
...
function AddInputLotDialog({ clientId, selectedLotIds, initialLine, onSave, onClose }: {
  clientId: string;
  selectedLotIds: string[];
  initialLine?: RequestLineDraft;
  onSave: (lot: EligibleProcessingLot, requestedKg: number, requestedBags: number) => void;
  onClose: () => void;
}) {
  const [sourceType, setSourceType] = useState<SourceType | null>(initialLine?.lot.source_type ?? null);
  const [search, setSearch] = useState("");
  const [lots, setLots] = useState<EligibleProcessingLot[]>([]);
  const [selectedLot, setSelectedLot] = useState<EligibleProcessingLot | null>(initialLine?.lot ?? null);
  const [requestedKg, setRequestedKg] = useState(initialLine?.requestedKg ?? 0);
  const [requestedBags, setRequestedBags] = useState(initialLine?.requestedBags ?? 0);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      setError("");
      void listEligibleProcessingLots(clientId, sourceType, search, 10).then((result) => {
        if (!canceled) setLots(result);
      }).catch((reason: unknown) => {
        if (!canceled) setError(reason instanceof Error ? reason.message : "Eligible lots could not be loaded.");
      }).finally(() => {
  ...
  function chooseLot(lot: EligibleProcessingLot) {
    if (selectedLotIds.includes(lot.lot_id)) {
      setError(`${lot.lot_number} has already been added. Edit its existing quantity instead.`);
      return;
    }
    setSelectedLot(lot);
    setRequestedKg(0);
  ...
  function save() {
    if (!selectedLot) { setError("Select an eligible coffee lot."); return; }
    if (requestedKg <= 0 || requestedKg > selectedLot.available_kg) { setError(`Enter a quantity up to ${selectedLot.available_kg.toLocaleString()} kg.`); return; }
    if (requestedBags <= 0 || requestedBags > selectedLot.available_bags) { setError(`Enter bags up to ${selectedLot.available_bags.toLocaleString()}.`); return; }
    onSave(selectedLot, requestedKg, requestedBags);
  }

  return <div className="modal-backdrop"><section className="receipt-modal input-lot-dialog" role="dialog" aria-modal="true" aria-labelledby="add-input-lot-title">
    <header><div><span className="demo-label">PROCESSING INPUT</span><h2 id="add-input-lot-title">{initialLine ? "Edit input lot" : "Add input lot"}</h2><p>Choose the coffee source first, then select an eligible lot.</p></div><button type="button" aria-label="Close input lot dialog" onClick={onClose}><X size={20} /></button></header>
    {!sourceType ? <section className="source-type-step"><h3>1. Select source type</h3><div className="source-type-options">{(["ARRIVAL", "REJECT", "PROCESSED"] as SourceType[]).map((type) => <button key={type} type="button" onClick={() => setSourceType(type)}><PackageCheck size={20} /><strong>{sourceTypeLabels[type]}</strong><small>{type === "ARRIVAL" ? "Received through a GRN" : `Created by processing as ${sourceTypeLabels[type].toLowerCase()} coffee`}</small></button>)}</div></section> : <>
      <section className="lot-picker-toolbar"><button className="secondary-button" type="button" onClick={() => { setSourceType(null); setSelectedLot(null); }}>Change source type</button><label><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={`Search ${sourceTypeLabels[sourceType].toLowerCase()} lots`} /></label></section>
      <section className="candidate-lots" aria-busy={loading}><h3>2. Select eligible {sourceTypeLabels[sourceType]} lot</h3>{loading && <p className="muted">Loading recent eligible lots…</p>}{!loading && lots.map((lot) => { const duplicate = selectedLotIds.includes(lot.lot_id); return <button className={selectedLot?.lot_id === lot.lot_id ? "selected" : ""} type="button" key={lot.lot_id} onClick={() => chooseLot(lot)} disabled={duplicate}><span><strong>{lot.lot_number}</strong><small>{lot.client_name}</small></span><span><strong>{lot.available_kg.toLocaleString()} kg</strong><small>{lot.available_bags.toLocaleString()} bags available</small></span><span><strong>{lot.source_document ?? "No source number"}</strong><small>{lot.origin ?? "Origin not recorded"} · {lot.grade}</small></span>{duplicate && <em>Already added</em>}</button>; })}{!loading && lots.length === 0 && !error && <p className="muted">No matching eligible lots are available.</p>}</section>
      {selectedLot && <section className="selected-lot-quantity"><h3>3. Quantity to process</h3><div className="selected-lot-summary"><strong>{selectedLot.lot_number}</strong><span>{sourceTypeLabels[selectedLot.source_type]} · {selectedLot.source_document ?? "No source number"}</span><span>Physical {selectedLot.quantity_kg.toLocaleString()} kg · Reserved {selectedLot.reserved_kg.toLocaleString()} kg · Available {selectedLot.available_kg.toLocaleString()} kg</span></div><div className="form-grid compact"><label>Quantity to process (kg)<input type="number" min="0.01" step="0.01" max={selectedLot.available_kg} value={requestedKg || ""} onChange={(event) => setRequestedKg(Number(event.target.value))} /></label><label>Bags to issue<input type="number" min="1" step="1" max={selectedLot.available_bags} value={requestedBags || ""} onChange={(event) => setRequestedBags(Number(event.target.value))} /></label></div></section>}
    </>}
    {error && <div className="request-form-error" role="alert"><AlertTriangle size={15} />{error}</div>}
    <footer><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="button" disabled={!selectedLot} onClick={save}>{initialLine ? "Update input" : "Add input lot"}</button></footer>
  </section></div>;
}

function ProcessingOrderDetail({ data, orderId, onClose }: { data: ProcessingData; orderId: string; onClose: () => void }) {
  const order = data.orders.find((item) => item.id === orderId);
  if (!order) return null;
  const request = order.request_id ? data.requests.find((item) => item.id === order.request_id) : undefined;
  ...
  const outputSources = new Map(outputs.map((output) => [output.id, data.outputSources.filter((source) => source.output_id === output.id).map((source) => inputById.get(source.input_id)).filter(Boolean)]));
  return <div className="modal-backdrop"><section className="receipt-modal processing-order-detail" role="dialog" aria-modal="true" aria-labelledby="processing-order-detail-title"><header><div><span className="demo-label">TRACEABLE PROCESSING</span><h2 id="processing-order-detail-title">{order.order_number}</h2><p>{client?.legal_name ?? "Unknown client"} · {order.status.replaceAll("_", " ")}</p></div><button type="button" aria-label="Close processing order details" onClick={onClose}><X size={20} /></button></header>
    <section><h3>Inputs</h3><div className="traceability-table"><div><strong>Type</strong><strong>Lot</strong><strong>Source</strong><strong>Quantity used</strong></div>{inputs.map((input) => { const lot = lotById.get(input.lot_id); const receipt = data.receipts.find((item) => item.id === lot?.receipt_id); return <div key={input.id}><span>{lot?.lot_category === "ARRIVAL" ? "Arrival" : lot?.lot_category === "CLIENT_REJECT" ? "Reject" : "Processed"}</span><span className="reference">{lot?.lot_number ?? input.lot_id}</span><span>{lot?.source_processing_order_id ? data.orders.find((item) => item.id === lot.source_processing_order_id)?.order_number : receipt?.receipt_number ?? "-"}</span><span>{Number(input.input_kg).toLocaleString()} kg / {input.input_bags} bags</span></div>; })}</div></section>
    <section><h3>Outputs</h3><div className="traceability-table"><div><strong>Type</strong><strong>Lot</strong><strong>Quantity</strong><strong>Source lots</strong></div>{outputs.map((output) => { const child = lotById.get(output.child_lot_id ?? ""); const sources = outputSources.get(output.id) ?? []; return <div key={output.id}><span>{output.category === "ACCEPTED_CLIENT_COFFEE" ? "Processed" : output.category === "CLIENT_REJECT" ? "Reject" : output.category.replaceAll("_", " ")}</span><span className="reference">{child?.lot_number ?? "No reusable lot"}</span><span>{Number(output.quantity_kg).toLocaleString()} kg</span><span>{sources.map((source) => lotById.get(source!.lot_id)?.lot_number).filter(Boolean).join(", ")}</span></div>; })}</div></section>
    <section className="processing-detail-summary"><h3>Processing summary</h3><dl><div><dt>Total input</dt><dd>{Number(order.input_kg).toLocaleString()} kg</dd></div><div><dt>Processed/rework</dt><dd>{Number(order.accepted_client_kg).toLocaleString()} kg</dd></div><div><dt>Reject</dt><dd>{Number(order.client_reject_kg).toLocaleString()} kg</dd></div><div><dt>Loss</dt><dd>{Number(order.process_loss_kg).toLocaleString()} kg</dd></div><div><dt>Reusable yield</dt><dd>{Number(order.input_kg) > 0 ? ((Number(order.accepted_client_kg) + Number(order.client_reject_kg)) / Number(order.input_kg) * 100).toFixed(2) : "0.00"}%</dd></div></dl></section>
    <section className="processing-file-timeline"><h3>Files for this processing journey</h3><p>Request, intake and completion evidence stay connected to this order.</p>{request && <EvidenceUploader reference={{ type: "PROCESSING_REQUEST", id: request.id, label: request.requestNumber ?? request.id }} documentType="PROCESSING_REQUEST" label="1. Request document" help="Client request, paper note, PDF or photo." />}<EvidenceUploader reference={{ type: "PROCESSING_ORDER", id: order.id, label: order.order_number }} documentType="PROCESSING_INTAKE" label="2. Intake evidence" help="Scale ticket, warehouse issue, or machine intake evidence." />{order.status === "POSTED" && <EvidenceUploader reference={{ type: "PROCESSING_ORDER", id: order.id, label: order.completion_number ?? order.order_number }} documentType="PROCESSING_COMPLETION" label="3. Completion evidence" help="Output weighing sheets, reconciliation and exception evidence." />}</section>
    <footer><button className="secondary-button" type="button" onClick={onClose}>Close</button></footer>
  </section></div>;
}

function EcxCheckDrawer({ request, check, onSaved, onClose }: { request: ProcessingRequest; check?: EcxCheckRow; onSaved: (message: string) => Promise<void>; onClose: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const saved = await saveEcxCheck({ id: savedCheck?.id, processingRequestId: request.id, checkedOn: String(form.get("checkedOn")), result: String(form.get("result")) as EcxCheckRow["result"], referenceNumber: String(form.get("referenceNumber") ?? ""), inspectorName: String(form.get("inspectorName") ?? ""), notes: String(form.get("notes") ?? "") });
      setSavedCheck(saved);
      await onSaved(`${saved.check_number} saved. ECX status is now ${saved.result.replaceAll("_", " ").toLowerCase()}.`);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "The ECX check could not be saved."); }
    finally { setBusy(false); }
  }
  return <RecordDetailDrawer open eyebrow="ECX CHECK" title={savedCheck?.check_number ?? "New ECX check"} subtitle={`${request.requestNumber} · ${request.client}`} status={<Status value={savedCheck?.result ?? "PENDING"} />} onClose={onClose}>
    <DetailGrid items={[{ label: "Processing request", value: request.requestNumber }, { label: "Paper note", value: request.noteNumber }, { label: "Coffee lot", value: request.lot }, { label: "Quantity", value: `${request.requestedKg.toLocaleString()} kg` }]} />
    <form className="drawer-form" onSubmit={submit}><label>Check date<input name="checkedOn" type="date" required defaultValue={savedCheck?.checked_on ?? new Date().toISOString().slice(0, 10)} /></label><label>Result<select name="result" defaultValue={savedCheck?.result ?? "PENDING"}><option value="PENDING">Pending</option><option value="PASSED">Passed</option><option value="FAILED">Failed</option><option value="NOT_REQUIRED">Not required</option></select></label><label>ECX reference<input name="referenceNumber" defaultValue={savedCheck?.reference_number ?? ""} placeholder="ECX reference or certificate" /></label><label>Inspector / checker<input name="inspectorName" defaultValue={savedCheck?.inspector_name ?? ""} /></label><label className="wide">Notes<textarea name="notes" rows={3} defaultValue={savedCheck?.notes ?? ""} /></label>{error && <p className="request-form-error wide" role="alert">{error}</p>}<button className="primary-button wide" type="submit" disabled={busy}><FileCheck2 size={16} />{busy ? "Saving check..." : savedCheck ? "Update ECX check" : "Save ECX check"}</button></form>
    <EvidenceUploader reference={savedCheck ? { type: "ECX_CHECK", id: savedCheck.id, label: savedCheck.check_number } : undefined} documentType="ECX_CHECK_EVIDENCE" label="ECX certificate or evidence" help="Add the PDF, photo, or scanned certificate after saving the check." />
    <EvidenceUploader reference={{ type: "PROCESSING_REQUEST", id: request.id, label: request.requestNumber ?? request.id }} documentType="PROCESSING_REQUEST" label="Processing request document" help="Attach the client's signed request, paper note, or supporting photo." />
  </RecordDetailDrawer>;
}
...
export function ProcessingOperations({ initialState = "All" }: { initialState?: ProcessingStateFilter }) {
  ...
  const [processingPurpose, setProcessingPurpose] = useState("Export preparation");
  ...
  const queue: QueueItem[] = (data?.orders ?? []).filter((item) => ["QUEUED", "BLOCKED"].includes(item.status)).sort((a, b) => a.queue_position - b.queue_position).map((item) => {
    ...
    const reservations = data?.reservations.filter((reservation) => reservation.processing_order_id === item.id && reservation.status === "ACTIVE") ?? [];
    const blocked = item.status === "BLOCKED" || inputs.length === 0 || inputs.some((input) => !reservations.some((reservation) => reservation.lot_id === input.lot_id && Number(reservation.reserved_kg) === Number(input.input_kg) && reservation.reserved_bags === input.input_bags));
    const inputLots = inputs.map((input) => lotById.get(input.lot_id)?.lot_number ?? input.lot_id).join(", ");
    const primaryLot = lotById.get(inputs[0]?.lot_id ?? item.lot_id);
    return { databaseId: item.id, id: item.order_number, position: item.queue_position, client: clientById.get(item.client_id) ?? "Unknown client", lot: inputLots || "No source lots", coffeeType: primaryLot?.coffee_type === "WASHED" ? "Washed" : "Unwashed / UG", grade: request?.grade ?? "-", inputBags: inputByOrder.get(item.id) ?? 0, inputKg: Number(item.input_kg), received: request?.requestDate ?? "-", readiness: blocked ? "BLOCKED" : "READY", note: blocked ? "Processing reservations need review" : request ? `${inputs.length} source lot(s) · ${request.preparationType}` : "Approved processing order" };
  });
  const orders: Order[] = (data?.orders ?? []).filter((item) => ["IN_PROCESS", "POSTED"].includes(item.status)).map((item) => {
    ...
    return { databaseId: item.id, id: item.order_number, position: item.queue_position, client: clientById.get(item.client_id) ?? "Unknown client", lot: inputs.map((input) => lotById.get(input.lot_id)?.lot_number ?? input.lot_id).join(", "), coffeeType: lot?.coffee_type === "WASHED" ? "Washed" : "Unwashed / UG", grade: requestById.get(item.request_id ?? "")?.grade ?? "-", inputBags: inputByOrder.get(item.id) ?? 0, inputKg: Number(item.input_kg), received: "-", readiness: "READY", note: `${inputs.length} source lot(s)`, status: item.status === "POSTED" ? "COMPLETED" : "IN_PROCESS", completionNumber: item.completion_number, machine: intake?.machine_line ?? "-", startedAt: item.started_at };
  });
  ...
  async function reload() {
    try { setData(await loadProcessingData()); setDatabaseError(""); }
    catch (error) { setData(null); setDatabaseError(error instanceof Error ? error.message : "Processing records could not be loaded."); }
  }
  ...
  function saveInputLot(lot: EligibleProcessingLot, requestedKg: number, requestedBags: number) {
    if (editingInputKey !== null) {
      setRequestLines((current) => current.map((line) => line.key === editingInputKey ? { ...line, lot, requestedKg, requestedBags } : line));
    } else {
      setRequestLines((current) => [...current, { key: rowKey++, lot, requestedKg, requestedBags }]);
    }
  ...
  async function createRequest(event: FormEvent<HTMLFormElement>) {
    ...
    const lines: ProcessingRequestLine[] = requestLines.map((line) => {
      return { lotDatabaseId: line.lot.lot_id, lot: line.lot.lot_number, coffeeType: line.lot.coffee_type === "WASHED" ? "Washed" : "Unwashed / UG", preparationType: processingPurpose, grade: line.lot.grade, requestedBags: line.requestedBags, requestedKg: line.requestedKg, certifications: selectedCertifications, specialInstruction: "", remark: "", clientDatabaseId: line.lot.client_id, availableKg: line.lot.available_kg, availableBags: line.lot.available_bags, sourceType: line.lot.source_type };
    });
    ...
    try {
      ...
      setSelectedClientId("");
      setRequestLines([]);
      setMessage(`${requestNumber} saved with ${lines.length} traceable input lot${lines.length === 1 ? "" : "s"}.`);
  ...
  const workflowRows = [
    ...requests.filter((request) => !request.queuedAs).map((request) => ({
      ...
      request, requestId: request.id, queueItem: null as QueueItem | null, order: null as Order | null,
    })),
    ...queue.map((queueItem) => ({
      ...
      request: null as ProcessingRequest | null, requestId: data?.orders.find((order) => order.id === queueItem.databaseId)?.request_id ?? "", queueItem, order: null as Order | null,
    })),
    ...orders.map((order) => ({
      ...
      request: null as ProcessingRequest | null, requestId: data?.orders.find((record) => record.id === order.databaseId)?.request_id ?? "", queueItem: null as QueueItem | null, order,
    })),
  ];
  ...
  return <div className="module-page processing-page">
    ...
    <WorkflowGuide title="Processing journey" steps={[{ label: "Request", help: "Record request, ECX check, and evidence", state: tab === "Requests" ? "current" : "done" }, { label: "Approve", help: "Independent approval and reservation", state: tab === "Queue" ? "current" : tab === "Requests" ? "next" : "done" }, { label: "Start", help: "Record intake and issue source lots", state: tab === "Intake" || tab === "Active Orders" ? "current" : ["Completion", "Output Lots", "Exceptions"].includes(tab) ? "done" : "next" }, { label: "Complete", help: "Reconcile outputs and create lots", state: ["Completion", "Output Lots", "Exceptions"].includes(tab) ? "current" : "next" }]} />
    ...
    {tab === "Requests" && <>
      ...
      <section className="record-panel"><div className="record-table processing-workflow-cols"><div className="table-head"><span>Order / Request</span><span>Client</span><span>Inputs</span><span>Quantity</span><span>Status</span><span>Next Action</span></div>{visibleWorkflowRows.map((item) => { const ecx = data?.ecxChecks.find((check) => check.processing_request_id === item.requestId); const ecxReady = ecx?.result === "PASSED" || ecx?.result === "NOT_REQUIRED"; return <div key={item.key}><span><strong className="reference">{item.reference}</strong>{item.request && <small>Paper note {item.request.noteNumber}</small>}</span><span>{item.client}</span><span>{item.inputs} lot{item.inputs === 1 ? "" : "s"}</span><span>{item.quantityKg.toLocaleString()} kg</span><span><Status value={item.statusLabel} />{item.requestId && <small>ECX: {ecx?.result.replaceAll("_", " ") ?? (item.order?.status === "COMPLETED" ? "Historical check missing" : "Action required")}</small>}</span><span className="request-actions">{item.requestId && <button className="table-action ecx-action" type="button" onClick={() => setEcxRequestId(item.requestId)}><FileCheck2 size={13} />{ecx ? "ECX details" : "Complete ECX check"}</button>}{item.request?.status === "DRAFT" && <button type="button" onClick={() => changeRequestStatus(item.request!.id, "SUBMITTED")}><Send size={13} />Submit request</button>}{item.request?.status === "SUBMITTED" && <><button type="button" disabled={!ecxReady} title={ecxReady ? "Approve processing request" : "Complete the ECX check first"} onClick={() => changeRequestStatus(item.request!.id, "APPROVED")}><ThumbsUp size={13} />{ecxReady ? "Approve request" : "ECX check required"}</button><button className="reject" type="button" aria-label={`Reject ${item.request.requestNumber}`} onClick={() => changeRequestStatus(item.request!.id, "REJECTED")}><ThumbsDown size={13} /></button></>}{item.request?.status === "APPROVED" && <button type="button" onClick={() => addRequestToQueue(item.request!)}>Queue for processing <ArrowRight size={13} /></button>}{item.request?.status === "REJECTED" && <span className="muted-action">No further action</span>}{item.queueItem && <><button className="table-action" type="button" onClick={() => setDetailOrderId(item.queueItem!.databaseId)}>Details & files</button><button className="table-action" type="button" disabled={item.queueItem.readiness === "BLOCKED"} onClick={() => openIntake(item.queueItem!)}>{item.queueItem.readiness === "BLOCKED" ? "Review reservations" : "Start Processing"}<ArrowRight size={13} /></button></>}{item.order && <><button className="table-action" type="button" onClick={() => setDetailOrderId(item.order!.databaseId)}>Details & files</button>{item.order.status === "IN_PROCESS" ? <button className="table-action" type="button" onClick={() => openCompletion(item.order!)}>Complete Processing <ArrowRight size={13} /></button> : <span className="muted-action">Locked</span>}</>}</span></div>; })}</div>{visibleWorkflowRows.length === 0 && <Empty title={`No ${stateFilter.toLowerCase()} processing records`} text="Choose another status or create a new processing request." />}</section>
    </>}
    ...
    {requestFormOpen && <div className="modal-backdrop"><form className="receipt-modal processing-request-modal" role="dialog" aria-modal="true" aria-labelledby="processing-request-title" onSubmit={createRequest}>
      <header><div><span className="demo-label">PAPER FORM DIGITIZATION</span><h2 id="processing-request-title">New processing request</h2><p>Add one or many eligible lots owned by one client.</p></div><button type="button" aria-label="Close request form" onClick={() => setRequestFormOpen(false)}><X size={20} /></button></header>
      <section className="form-section"><h3>Request header</h3><div className="form-grid compact request-form-grid"><label>Paper note number<input name="noteNumber" placeholder="00240" required /></label><label>Request date<input name="requestDate" type="date" defaultValue={new Date().toISOString().slice(0, 10)} required /></label><label>Client / customer<select required value={selectedClientId} onChange={(event) => { setSelectedClientId(event.target.value); setRequestError(""); setRequestLines([]); }}><option value="" disabled>Select client</option>{clients.filter((client) => client.active).map((client) => <option key={client.id} value={client.id}>{client.code} - {client.legal_name}</option>)}</select></label><label>Processing purpose<input required value={processingPurpose} onChange={(event) => setProcessingPurpose(event.target.value)} placeholder="Export preparation" /></label></div></section>
      <section className="form-section processing-inputs-section"><div className="section-title-row"><div><h3>Processing inputs</h3><p className="form-note muted">Arrival, Reject, and Processed lots can be combined when they belong to this client.</p></div><span className="info-badge muted">{requestLines.length} selected</span></div>
        {requestLines.length === 0 ? <div className="empty-input-lots"><PackageCheck size={22} /><strong>No input lots selected</strong><p>Start with one source lot and add more only when the processing order needs them.</p></div> : <div className="request-input-table"><div className="table-head"><span>Type</span><span>Lot</span><span>Source</span><span>Available</span><span>Selected</span><span>Control</span></div>{requestLines.map((line) => <div key={line.key}><span><Status value={sourceTypeLabels[line.lot.source_type]} /></span><span><strong className="reference">{line.lot.lot_number}</strong><small>{line.lot.grade}</small></span><span>{line.lot.source_document ?? "-"}<small>{line.lot.origin ?? "Origin not recorded"}</small></span><span>{line.lot.available_kg.toLocaleString()} kg<small>{line.lot.available_bags} bags</small></span><span>{line.requestedKg.toLocaleString()} kg<small>{line.requestedBags} bags</small></span><span className="row-actions"><button type="button" onClick={() => { setEditingInputKey(line.key); setInputPickerOpen(true); }}><Pencil size={13} />Edit</button><button className="reject" type="button" aria-label={`Remove ${line.lot.lot_number}`} onClick={() => setRequestLines((current) => current.filter((item) => item.key !== line.key))}><Trash2 size={13} /></button></span></div>)}</div>}
        <button className="secondary-button add-input-lot-button" type="button" disabled={!selectedClientId} onClick={() => { setEditingInputKey(null); setInputPickerOpen(true); }}><Plus size={15} />Add Input Lot</button>
      </section>
      <section className="form-section"><h3>Certification</h3><div className="certification-grid">{certifications.map((certification) => <label key={certification}><input type="checkbox" name="certifications" value={certification} />{certification}</label>)}</div><label className="other-certification">Other certification<input name="otherCertification" /></label></section>
      <section className="form-section"><h3>Request control</h3><div className="form-grid compact request-form-grid"><label>Representative / requester<select name="requester" required defaultValue=""><option value="" disabled>Select representative</option>{(data?.representatives ?? []).filter((item) => item.client_id === selectedClientId && item.active).map((item) => <option key={item.id}>{item.full_name}</option>)}</select></label><label>Checker<select name="checker" required defaultValue=""><option value="" disabled>Select checker</option>{staff.map((profile) => <option key={profile.id}>{profile.full_name}</option>)}</select></label><label>Approver<select name="approver" required defaultValue=""><option value="" disabled>Select approver</option>{staff.map((profile) => <option key={profile.id}>{profile.full_name}</option>)}</select></label><label className="wide">Notes<textarea name="notes" rows={3} /></label><div className="wide upload-after-save-note"><FileCheck2 size={17} /><span><strong>Save first, then attach the request file.</strong> Open “Complete ECX check” to upload the signed PDF, scan, or photo.</span></div></div></section>
      {requestError && <div className="request-form-error" role="alert"><AlertTriangle size={15} />{requestError}</div>}
      <footer><button className="secondary-button" type="button" onClick={() => setRequestFormOpen(false)}>Cancel</button><button className="primary-button" type="submit" disabled={!selectedClientId || requestLines.length === 0}><FileCheck2 size={16} />Save draft</button></footer>
    </form></div>}
    {inputPickerOpen && selectedClientId && <AddInputLotDialog clientId={selectedClientId} selectedLotIds={requestLines.filter((line) => line.key !== editingInputKey).map((line) => line.lot.lot_id)} initialLine={requestLines.find((line) => line.key === editingInputKey)} onSave={saveInputLot} onClose={() => { setInputPickerOpen(false); setEditingInputKey(null); }} />}

    {tab === "Queue" && <><section className="queue-rule"><Clock3 size={18} /><div><strong>Protected processing sequence</strong><p>Queueing reserves every source lot. Intake consumes all reservations in one transaction.</p></div></section><section className="record-panel"><div className="record-table queue-cols"><div className="table-head"><span>Queue / order</span><span>Client / lots</span><span>Coffee</span><span>Received</span><span>Readiness</span><span>Control</span></div>{queue.map((item) => <div key={item.id}><span><span className="queue-position">{item.position}</span><small>{item.id}</small></span><span><strong>{item.client}</strong><small>{item.lot}</small></span><span>{item.coffeeType}<small>{item.inputBags} bags - {item.inputKg.toLocaleString()} kg</small></span><span>{item.received}</span><span><Status value={item.readiness} /><small>{item.note}</small></span><span className="request-actions"><button className="table-action" type="button" onClick={() => setDetailOrderId(item.databaseId)}>Details</button><button className="table-action" type="button" disabled={item.readiness === "BLOCKED"} onClick={() => openIntake(item)}>Record intake <ArrowRight size={13} /></button></span></div>)}</div></section></>}
    ...
    {tab === "Active Orders" && <><section className="processing-summary"><article><Factory size={19} /><span>Active orders<strong>{activeOrders.length}</strong></span></article><article><Scale size={19} /><span>Input in process<strong>{activeOrders.reduce((sum, item) => sum + item.inputKg, 0).toLocaleString()} kg</strong></span></article><article><PackageCheck size={19} /><span>Completed orders<strong>{orders.filter((item) => item.status === "COMPLETED").length}</strong></span></article></section><section className="record-panel"><div className="record-table order-cols"><div className="table-head"><span>Order</span><span>Client / lots</span><span>Input</span><span>Machine</span><span>Started</span><span>Status</span><span>Action</span></div>{orders.map((item) => <div key={item.id}><span className="reference">{item.id}<small>{item.completionNumber ?? "No completion yet"}</small></span><span><strong>{item.client}</strong><small>{item.lot}</small></span><span>{item.inputKg.toLocaleString()} kg<small>{item.coffeeType} · {item.note}</small></span><span>{item.machine}</span><span>{item.startedAt ? new Date(item.startedAt).toLocaleString() : "-"}</span><span><Status value={item.status} /></span><span className="request-actions"><button className="table-action" type="button" onClick={() => setDetailOrderId(item.databaseId)}>Details</button><button className="table-action" type="button" disabled={item.status === "COMPLETED"} onClick={() => openCompletion(item)}>{item.status === "COMPLETED" ? "Locked" : "Record outputs"}<ArrowRight size={13} /></button></span></div>)}</div></section></>}
    ...
    {data && detailOrderId && <ProcessingOrderDetail data={data} orderId={detailOrderId} onClose={() => setDetailOrderId("")} />}
    {data && ecxRequestId && (() => { const request = data.requests.find((item) => item.id === ecxRequestId); return request ? <EcxCheckDrawer request={request} check={data.ecxChecks.find((item) => item.processing_request_id === ecxRequestId)} onSaved={async (nextMessage) => { await reload(); setMessage(nextMessage); }} onClose={() => setEcxRequestId("")} /> : null; })()}
  </div>;
}
