...
function MasterRecordModal({ kind, clientOptions, initialClientId, onSaved, onClose }: {
  ...
  return <div className="modal-backdrop" role="presentation"><form className="receipt-modal master-record-modal" onSubmit={submit} aria-label={title}>
    ...
    <div className="form-grid">
      {kind === "client" ? <>
        ...
        <label className="wide">Client<select name="clientId" required defaultValue={initialClientId ?? ""}><option value="" disabled>Select client</option>{clientOptions.filter((client) => client.status !== "INACTIVE").map((client) => <option key={client.id} value={client.id}>{client.code} - {client.name}</option>)}</select></label>
        {kind === "agreement" ? <>
...
export function CoreOperations({ activeView, userId, stockIntent, onNavigate }: { activeView: string; userId?: string; stockIntent?: { type: StockTypeFilter; status: StockStatusFilter; focusId?: string }; onNavigate?: (intent: { view: string; focusId?: string }) => void }) {
  ...
  const [masterModal, setMasterModal] = useState<{ kind: MasterRecordKind; clientId?: string } | null>(null);
